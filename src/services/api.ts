        ...
        switch (error.response.data?.code) {
          case "token.expired":
            ...
            if (!isRefreshing) {
              ...
              isRefreshing = true;
              const { "nextauth.refreshToken": refreshToken } =
                parseCookies(context);

              api
                .post<TRefreshResponse>("/refresh", { refreshToken })
                .then((response) => {
                  setCookie(context, "nextauth.token", response.data.token, {
                ...
                .catch((err) => {
                  ...
                  // typeof window é undefined caso esteja no server-side
                  // Como o signOut executa o Router.push, que só está disponível no client-side
                  // essa verificacão faz com que a func signOut só seja executada no client
